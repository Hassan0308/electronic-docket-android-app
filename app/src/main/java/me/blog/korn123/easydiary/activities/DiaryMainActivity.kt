...
//            mBinding.diaryListView.setSelection(0)
    ...
    override fun onOptionsItemSelected(item: MenuItem): Boolean {
        when (item.itemId) {
            android.R.id.home -> {
                if (mDiaryMode == DiaryMode.DELETE) {
                    // Switch back to DiaryMode.READ when the back arrow is clicked
                    ...
                    // Toggle the navigation drawer when in DiaryMode.READ
...
//                    }
    ...
    private fun bindEvent() {
        val navMenuCustomLayout: LinearLayout? = mBinding.navView?.findViewById(R.id.linearNav)

        val itemSettings = navMenuCustomLayout!!.findViewById<LinearLayoutCompat>(R.id.settings)
        val openDashboard = navMenuCustomLayout.findViewById<LinearLayoutCompat>(R.id.openDashboard)
        val calander = navMenuCustomLayout.findViewById<LinearLayoutCompat>(R.id.calander)
        val timeline = navMenuCustomLayout.findViewById<LinearLayoutCompat>(R.id.timeline)
        val gridOption = navMenuCustomLayout.findViewById<LinearLayoutCompat>(R.id.gridLayout)
        val postCard = navMenuCustomLayout.findViewById<LinearLayoutCompat>(R.id.postCard)
        val attachedPhotoGallery = navMenuCustomLayout.findViewById<LinearLayoutCompat>(R.id.attachedPhotoGallery)
        val stats = navMenuCustomLayout.findViewById<LinearLayoutCompat>(R.id.chart)

        mBinding.microphone?.setOnClickListener {
            showSpeechDialog()
        }
...
//                    mDiaryMainItemAdapter?.notifyDataSetChanged()
        ...
        attachedPhotoGallery.setOnClickListener {
            mBinding.drawerLayout!!.closeDrawer(GravityCompat.START)
            Intent(this@DiaryMainActivity, GalleryActivity::class.java).run {
                TransitionHelper.startActivityWithTransition(this@DiaryMainActivity, this)
            }
