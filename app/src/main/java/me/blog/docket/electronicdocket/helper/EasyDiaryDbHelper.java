package me.blog.docket.electronicdocket.helper;
...
public class EasyDiaryDbHelper {
    ...
    public static Realm getRealmInstance() {
        if (diaryConfig == null) {
            diaryConfig = new RealmConfiguration.Builder().allowWritesOnUiThread(true)
                    .name("diary.realm")
                    .schemaVersion(6)
    ...
    public static List<DiaryDto> readDiary(String query) {
        ...
        if (StringUtils.isEmpty(query)) {
            results = getRealmInstance().where(DiaryDto.class).sort("currentTimeMillis", Sort.DESCENDING).findAll();
        } else {
            results = getRealmInstance().where(DiaryDto.class).beginGroup().contains("contents", query).or().contains("title", query).endGroup().sort("currentTimeMillis", Sort.DESCENDING).findAll();
        }
    ...
    public static List<DiaryDto> readDiaryByDateString(String dateString) {
        RealmResults<DiaryDto> results = null;
        results = getRealmInstance().where(DiaryDto.class).equalTo("dateString", dateString).sort("sequence", Sort.DESCENDING).findAll();
        List<DiaryDto> list = new ArrayList<>();
        list.addAll(results.subList(0, results.size()));
        return list;
    }
